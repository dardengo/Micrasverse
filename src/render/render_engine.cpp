...
RenderEngine::RenderEngine(const std::shared_ptr<micrasverse::simulation::SimulationEngine>& simulationEngine) : simulationEngine(simulationEngine) {
    ...
    this->micrasRender = std::make_unique<MicrasRender>(
        this->simulationEngine->physicsEngine->getMicras().getPosition(), this->simulationEngine->physicsEngine->getMicras().getSize()
    );
    ...
    this->argbsSceneObjs.emplace_back(std::make_unique<ARGBRender>(
        b2Vec2(argb0.getWorldPosition().x, argb0.getWorldPosition().y), b2Vec2(argb0.getSize().x, argb0.getSize().y), argb0.getLightColorArray(),
        argb0.getBaseColorArray(), argb0.isOn(), screen->camera
    ));

    try {
        auto& argb1 = this->simulationEngine->physicsEngine->getMicras().getArgb(1);
        this->argbsSceneObjs.emplace_back(std::make_unique<ARGBRender>(
            b2Vec2(argb1.getWorldPosition().x, argb1.getWorldPosition().y), b2Vec2(argb1.getSize().x, argb1.getSize().y), argb1.getLightColorArray(),
            argb1.getBaseColorArray(), argb1.isOn(), screen->camera
        ));
    ...
    for (size_t i = 0; i < 4; i++) {
        ...
        this->lidarsSceneObjs.emplace_back(std::make_unique<LidarRender>(
            b2Vec2(lidar.getRayMidPoint().x, lidar.getRayMidPoint().y), b2MakeRot(lidar.getAngle()), lidar.getReadingVisual(), screen->camera
        ));
    }
...
void RenderEngine::update() {
    ...
    micrasRender->update(
        this->simulationEngine->physicsEngine->getMicras().getPosition(), this->simulationEngine->physicsEngine->getMicras().getRotation()
    );
    ...
    if (isFullscreen) {
        ...
        if (isObjectInView(micrasRender->getBoundingBox())) {
            ...
            this->argbsSceneObjs[0]->update(
                b2Vec2(argb.getWorldPosition().x, argb.getWorldPosition().y), this->simulationEngine->physicsEngine->getMicras().getRotation(),
                argb.getLightColorArray(), argb.isOn()
            );
            if (this->argbsSceneObjs.size() > 1) {
                auto& argb1 = this->simulationEngine->physicsEngine->getMicras().getArgb(1);
                this->argbsSceneObjs[1]->update(
                    b2Vec2(argb1.getWorldPosition().x, argb1.getWorldPosition().y), this->simulationEngine->physicsEngine->getMicras().getRotation(),
                    argb1.getLightColorArray(), argb1.isOn()
                );
            }
        }
    } else {
    ...
    if (isFullscreen) {
        ...
        for (size_t i = 0; i < 4; i++) {
            ...
            if (reading > 0.005f) {
                ...
                this->lidarsSceneObjs[i]->update(
                    b2Vec2(
                        this->simulationEngine->physicsEngine->getMicras().getDistanceSensor(i).getRayMidPoint().x,
                        this->simulationEngine->physicsEngine->getMicras().getDistanceSensor(i).getRayMidPoint().y
                    ),
                    b2MakeRot(std::atan2(
                        this->simulationEngine->physicsEngine->getMicras().getDistanceSensor(i).getRayDirection().y,
        ...
        for (size_t i = 0; i < 4; i++) {
            ...
            this->lidarsSceneObjs[i]->update(
                ...
                b2MakeRot(std::atan2(lidar.getRayDirection().y, lidar.getRayDirection().x)), lidar.getReadingVisual()
            );
        }
    }
...
bool RenderEngine::isObjectInView(const BoundingBox& bbox) {
    ...
    std::array<glm::vec4, 8> corners = {glm::vec4(bbox.min.x, bbox.min.y, bbox.min.z, 1.0f), glm::vec4(bbox.max.x, bbox.min.y, bbox.min.z, 1.0f),
                                        glm::vec4(bbox.min.x, bbox.max.y, bbox.min.z, 1.0f), glm::vec4(bbox.max.x, bbox.max.y, bbox.min.z, 1.0f),
                                        glm::vec4(bbox.min.x, bbox.min.y, bbox.max.z, 1.0f), glm::vec4(bbox.max.x, bbox.min.y, bbox.max.z, 1.0f),
                                        glm::vec4(bbox.min.x, bbox.max.y, bbox.max.z, 1.0f), glm::vec4(bbox.max.x, bbox.max.y, bbox.max.z, 1.0f)};
...
void RenderEngine::renderFrame() {
    ...
    mazeRender->render(screen->view, screen->projection, lightPosition, screen->camera.position, lightAmbient, lightDiffuse, lightSpecular);

    // 2. Render Micras robot (uses same shader as maze)
    micrasRender->render(screen->view, screen->projection, lightPosition, screen->camera.position, lightAmbient, lightDiffuse, lightSpecular);
...
void RenderEngine::reloadMaze(const std::string& mazeFilePath) {
    ...
    for (size_t i = 0; i < 4; i++) {
        auto& lidar = this->simulationEngine->physicsEngine->getMicras().getDistanceSensor(i);
        this->lidarsSceneObjs[i]->update(
            b2Vec2(lidar.getRayMidPoint().x, lidar.getRayMidPoint().y), b2MakeRot(std::atan2(lidar.getRayDirection().y, lidar.getRayDirection().x)),
