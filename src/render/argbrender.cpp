...
ARGBRender::ARGBRender(
    const b2Vec2 position, const b2Vec2 size, const glm::vec3 lightColor, const glm::vec3 baseColor, const bool isOn, const Camera& camera
) :
    ...
    this->renderModel =
        Led(isOn ? this->lightColor : this->baseColor, glm::vec3(1.0f), glm::vec3(1.0f), glm::vec3(1.0f), glm::vec3(position.x, position.y, 0.0004f),
...
ARGBRender::ARGBRender(
    const b2Vec2 position, const b2Vec2 size, const std::array<float, 3>& lightColor, const std::array<float, 3>& baseColor, const bool isOn,
    const Camera& camera
) :
    LightObj(), camera(camera) {
