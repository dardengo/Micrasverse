...
std::unique_ptr<IArgb> Box2DPhysicsFactory::createARGB(const types::Vec2& localPosition, const types::Vec2& size, const types::Color& color) {
...
std::unique_ptr<IMotor> Box2DPhysicsFactory::createMotor(
    const types::Vec2& localPosition, bool leftWheel, float angle, float resistance, float ke, float kt, float maxVoltage
) {
    return std::make_unique<Box2DMotor>(
        bodyId, localPosition, leftWheel, angle, resistance != 0.0f ? resistance : MOTOR_RESISTANCE, ke != 0.0f ? ke : MOTOR_KE,
        kt != 0.0f ? kt : MOTOR_KT, maxVoltage != 0.0f ? maxVoltage : MOTOR_MAX_VOLTAGE
    );
}
